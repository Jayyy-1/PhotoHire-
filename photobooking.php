        ...
        <div class="row g-3">
            ...
            <div class="col-md-6">
                ...
                <select class="form-select" id="package" name="package" required>
                    ...
                    <option value="Gold II Package">Gold II Package - ₱5,449</option>
                    <option value="Custom Package">Create Your Own Package</option>
                </select>
            </div>
            <div class="col-md-6">
    <label for="event_date" class="form-label">Date of Event</label>
    <input type="date" class="form-control" id="event_date" name="event_date" required onchange="checkDateAvailability()">
    <small id="date-feedback" class="form-text text-danger"></small>
</div>
...
function checkDateAvailability() {
    ...
    if (eventDate) {
        // Make an AJAX request to check date availability
        fetch(`check_date.php?date=${eventDate}`)
            .then(response => response.json())
            .then(data => {
                if (data.available) {
                    feedback.textContent = ""; // Clear feedback if date is available
                } else {
                    feedback.textContent = "This date is already booked. Please select another date.";
                }
            })
            .catch(err => {
                feedback.textContent = "Error checking date availability.";
                console.error(err);
            });
    }
}
...
<script>
    document.getElementById('package').addEventListener('change', function () {
        ...
        customFields.style.display = this.value === 'Custom Package' ? 'block' : 'none';
    });
</script>
</body>
</html>
